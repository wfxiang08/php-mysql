...
static int mysql_handle_preparer(pdo_dbh_t *dbh, const char *sql, size_t sql_len, pdo_stmt_t *stmt,
                                 zval *driver_options) {
    pdo_mysql_db_handle *H = (pdo_mysql_db_handle *) dbh->driver_data;
...
/* {{{ mysql_handle_quoter */
static int mysql_handle_quoter(pdo_dbh_t *dbh, const char *unquoted, size_t unquotedlen, char **quoted,
                               size_t *quotedlen, enum pdo_param_type paramtype) {
    pdo_mysql_db_handle *H = (pdo_mysql_db_handle *) dbh->driver_data;
...
static int pdo_mysql_get_attribute(pdo_dbh_t *dbh, zend_long attr, zval *return_value) {
    ...
    switch (attr) {
        ...
        case PDO_MYSQL_ATTR_USE_BUFFERED_QUERY: ZVAL_LONG(return_value, H->buffered);
            break;

        case PDO_ATTR_EMULATE_PREPARES:
        case PDO_MYSQL_ATTR_DIRECT_QUERY: ZVAL_LONG(return_value, H->emulate_prepare);
            break;
