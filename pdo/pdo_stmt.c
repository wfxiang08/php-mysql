...
static int really_register_bound_param(struct pdo_bound_param_data *param, pdo_stmt_t *stmt, int is_param) /* {{{ */
{
    ...
    if (PDO_PARAM_TYPE(param->param_type) == PDO_PARAM_STR && param->max_value_len <= 0 && !Z_ISNULL_P(parameter)) {
        if (Z_TYPE_P(parameter) == IS_DOUBLE) {
            ...
            convert_to_string(parameter);
               ...
               (Z_TYPE_P(parameter) == IS_FALSE || Z_TYPE_P(parameter) == IS_TRUE)) {
    ...
    if (!is_param && param->name && stmt->columns) {
        ...
        if (param->paramno == -1) {
            char *tmp;
            spprintf(&tmp, 0, "Did not find column name '%s' in the defined columns; it will not be bound",
                     ZSTR_VAL(param->name));
...
static int do_fetch_class_prepare(pdo_stmt_t *stmt) /* {{{ */
{
    ...
    if (ce->constructor) {
        ...
        pdo_raise_impl_error(stmt->dbh, stmt, "HY000",
                             "user-supplied class does not have a constructor, use NULL for the ctor_params parameter, or simply omit it");
        return 0;
    } else {
...
static int make_callable_ex(pdo_stmt_t *stmt, zval *callable, zend_fcall_info *fci, zend_fcall_info_cache *fcc,
                            int num_args) /* {{{ */
{
...
static int do_fetch(pdo_stmt_t *stmt, int do_bind, zval *return_value, enum pdo_fetch_type how,
                    enum pdo_fetch_orientation ori, zend_long offset, zval *return_all) /* {{{ */
{
            ...
            switch (how) {
                ...
                case PDO_FETCH_KEY_PAIR: {
                    ...
                    if (Z_TYPE(val) == IS_LONG) {
                        zend_hash_index_update((return_all ? Z_ARRVAL_P(return_all) : Z_ARRVAL_P(return_value)),
                                               Z_LVAL(val), &tmp);
                    } else {
                        convert_to_string(&val);
                        zend_symtable_update((return_all ? Z_ARRVAL_P(return_all) : Z_ARRVAL_P(return_value)),
                                             Z_STR(val), &tmp);
                    }
                    zval_ptr_dtor(&val);
                    return 1;
                    ...
                    if ((flags & PDO_FETCH_SERIALIZE) == 0 || idx) {
                        ...
                        if (!ce->unserialize) {
                            ...
                            return 0;
                                                   ...
                                                   (unsigned char *) (Z_TYPE(val) == IS_STRING ? Z_STRVAL(val) : ""),
...
static int pdo_stmt_verify_mode(pdo_stmt_t *stmt, zend_long mode, int fetch_all) /* {{{ */
{
    ...
    switch (mode) {
        case PDO_FETCH_FUNC:
            if (!fetch_all) {
                pdo_raise_impl_error(stmt->dbh, stmt, "HY000",
                                     "PDO::FETCH_FUNC is only allowed in PDOStatement::fetchAll()");
                return 0;
            }
            return 1;
        ...
        ZEND_HASH_FOREACH_KEY_PTR(stmt->bound_params, num, key, param)
                {
                    ...
                    php_stream_printf(out, "paramno=%pd\nname=[%zd] \"%.*s\"\nis_param=%d\nparam_type=%d\n",
                                      param->paramno, param->name ? ZSTR_LEN(param->name) : 0,
                                      param->name ? (int) ZSTR_LEN(param->name) : 0,
...
static union _zend_function *dbstmt_method_get(zend_object **object_pp, zend_string *method_name, const zval *key) {
    ...
    if ((fbc = zend_hash_find_ptr(&object->ce->function_table, lc_method_name)) == NULL) {
        ...
        if ((fbc = zend_hash_find_ptr(stmt->dbh->cls_methods[PDO_DBH_DRIVER_METHOD_KIND_STMT], lc_method_name)) ==
            NULL) {
            goto out;
        }
        /* got it */
